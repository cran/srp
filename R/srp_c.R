#' The Smooth-Rough Partition linear regression model fitting
#'
...
#' @param L An integer specifying the dimension of b-spline expansion for constrained parameters. The default is 35.
...
srp.c <- function(x, y, maxq=10, L=35, norder=4, plot=T){
  ...
  est_q <- sic.c(x.basis=x.basis, B.basis=B.basis, x=x, y=y, cf0=cf0, maxq=maxq, fixedq=F, L=L)
  qhat <- est_q$qhat
  sicq <- est_q$sicq

  fit <- sic.c(x.basis=x.basis, B.basis=B.basis, x=x, y=y, cf0=cf0, maxq=qhat, fixedq=T, L=L)
